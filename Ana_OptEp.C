...
int main(int argc, char** argv)
{
  ...
  //else Ana();
  TFile *fileout = new TFile("output/output_optep.root","new");
...
//////////////////////////////
    if (fabs(tof11-tof21)<3 && p2<m_pcut){
	  for (int epi=0; epi<nep; epi++) {
        if (epratio1<epn[epi] && epratio2<epn[epi]) datasel[epi]->Fill();
	  }
    }
//////////////////////////////
...
//cutflow<<"m2 fit signal     :"<<sigm2 <<std::endl;
  ...
  char nameep[100];
  for (int epi=0; epi<nep; epi++){
    //sprintf(nameep,"%s_%.2f",&pureName[9],epn[epi]);
    sprintf(nameep,"%.2f",epn[epi]);
    std::cout<<nameep<<" entries is " << datasel[epi]->GetEntries()<<std::endl;
    FitSpectrum(datasel[epi], Ebeam, nameep);
  }
...
double FitSpectrum(TTree *&dataraw, double beame, const char* namesfx)
{
 ...
 //  RooGaussian gausb("gausb","gauss(x,m,s)",x,meanb,sigmab);
   ...
   //sum->fitTo(*dataset);
   RooFitResult *fitresult = sum->fitTo(*dataset,"e",Range("sigragi"));
   ...
   //sum->plotOn(xframe  ,Range(peakvalue-0.1, peakvalue+0.05)  );
   sum->plotOn(xframe);
   xframe->Draw();
 ...
 //outf<<beame<<"\t"<<mean.getVal()<<"\t"<<mean.getError()<<std::endl;
   ...
   fitpar<<"\t bkg mean = " << meanb.getVal() << "\t bkg sigma = " << sigmab.getVal() << std::endl;;
   ...
   RooAbsReal* intsum3 = sum->createIntegral(x,  NormSet(x),  Range("sigrag3"));
   RooAbsReal* intsum5 = sum->createIntegral(x,  NormSet(x),  Range("sigrag5"));
   double signalN3 =   signal.getVal()*intsig3->getVal();
   double signalE3 =   signal.getError()*intsig3->getVal();
   //double signalE3 =   signal.getVal()*intsig3->getPropagatedError(*fitresult);
   double backN3 = background.getVal()*intbck3->getVal();
   double backE3 = background.getError()*intbck3->getVal();
   //double backE3 = background.getVal()*intbck3->getPropagatedError(*fitresult);
   double sumN3 = (signal.getVal()+background.getVal())*intsum3->getVal();
   double sumE3 = sqrt(pow(signalE3,2)+pow(backE3,2)); //(signal.getError()+background.getError())*intsum3->getVal();
   //double sumE3 = (signal.getVal()+background.getVal())*intsum3->getPropagatedError(*fitresult);
   double signalN5 =   signal.getVal()*intsig5->getVal();
   double signalE5 =   signal.getError()*intsig5->getVal();
   //double signalE5 =   signal.getVal()*intsig5->getPropagatedError(*fitresult);
   double backN5 = background.getVal()*intbck5->getVal();
   double backE5 = background.getError()*intbck5->getVal();
   //double backE5 = background.getVal()*intbck5->getPropagatedError(*fitresult);
   double sumN5 = (signal.getVal()+background.getVal())*intsum5->getVal();
   //double sumE5 = (signal.getError()+background.getError())*intsum5->getVal();
   double sumE5 = sqrt(pow(signalE5,2)+pow(backE5,2)); //(signal.getError()+background.getError())*intsum3->getVal();
   //double sumE5 = (signal.getVal()+background.getVal())*intsum5->getPropagatedError(*fitresult);
 ...
 //double backN5nom = background.getVal()*intbck5->getVal()/intbcki->getVal();
   ofstream epSNR("epSNR2.dat",std::ios::app);
   epSNR << namesfx <<"\t"<<signalN3 << "\t"<<signalE3 ;
   epSNR << "\t" << backN3 << "\t" << backE3 ;
   epSNR << "\t" << sumN3  << "\t" << sumE3 ;
   epSNR << "\t" <<signalN5<< "\t" << signalE5;
   epSNR << "\t" << backN5 << "\t" << backE5;
   epSNR << "\t" << sumN5  << "\t" << sumE5;
   epSNR << std::endl;
