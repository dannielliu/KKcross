...
int main(int argc, char** argv)
{
  ...
  //else Ana();
  TFile *fileout = new TFile("output_cut/output_mc.root","update");
...
//datasel2->Branch("mass",&mass,"mass/D");
  ...
  vars->Draw("p1>>hpdis");
  vars->Draw("p1:p2>>h2pdis");
  sprintf(name2,"h1p");
  dir->WriteTObject(hpdis,name2);
  sprintf(name2,"h2p");
  dir->WriteTObject(h2pdis,name2);
  
  TH1D* hthetadis = new TH1D("hthetadis","#theta distribution",100,170,180);
  vars->Draw("theta>>hthetadis");
  sprintf(name2,"htheta");
  dir->WriteTObject(hthetadis,name2);
  
  TH1D* htheta1dis = new TH1D("htheta1dis","#theta1 distribution",1000,-1,1);
  vars->Draw("costheta1>>htheta1dis");
  sprintf(name2,"hcostheta1");
  dir->WriteTObject(htheta1dis,name2);
  
  TH1D* htheta2dis = new TH1D("htheta2dis","#theta2 distribution",1000,-1,1);
  vars->Draw("costheta2>>htheta2dis");
  sprintf(name2,"hcostheta2");
  dir->WriteTObject(htheta2dis,name2);
  
  TH1D* hepdis = new TH1D("hepdis","E/p distribution",1000,0,1.5);
  TH2D* h2epdis = new TH2D("h2epdis","E/p distribution",1000,0,1.5,1000,0,1.5);
  vars->Draw("ep1>>hepdis");
  vars->Draw("ep1:ep2>>h2epdis");
  sprintf(name2,"hep");
  dir->WriteTObject(hepdis,name2);
  sprintf(name2,"h2ep");
  dir->WriteTObject(h2epdis,name2);

  TH1D* htotep = new TH1D("htotep","htotep",100,0,2.5);
  vars->Draw("ep1+ep2>>htotep");
  sprintf(name2,"totep");
  dir->WriteTObject(htotep,name2);
  
  TH1D* hdtofdis = new TH1D("hdtofdis","#delta{TOF} distribution",1000,-20,20);
  vars->Draw("tof1-tof2>>hdtofdis");
  sprintf(name2,"hdtof");
  dir->WriteTObject(hdtofdis,name2);
