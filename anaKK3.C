...
int Ana(const char *filename, const char* outdir, TFile *fileout)
{
  ...
  double mckappx,mckappy,mckappz,mckampx,mckampy,mckampz;
  int nneu;
  int run;
  int idxmc;
  int pdgid[100];
  ...
  tree->SetBranchAddress("mc_kappx",&mckappx);
  tree->SetBranchAddress("mc_kappy",&mckappy);
  tree->SetBranchAddress("mc_kappz",&mckappz);
  tree->SetBranchAddress("mc_kampx",&mckampx);
  tree->SetBranchAddress("mc_kampy",&mckampy);
  tree->SetBranchAddress("mc_kampz",&mckampz);
...
//TTree *vars = new TTree("vars","vars");
  ...
  //kap.SetVectMag(TVector3(mckappx,mckappy,mckappz),mka);
  //kam.SetVectMag(TVector3(mckampx,mckampy,mckampz),mka);
...
//cutflow<<"m2 fit signal     :"<<sigm2 <<std::endl;
  ...
  // show some distribution and cut
  char name2[100];
  sprintf(name2,"hlcos");
  ...
  sprintf(cut,"abs(p2-%f)<0.03 & abs(costheta1)<0.93 & abs(costheta2)<0.93",pexp);
  vars->Draw("p1>>hpdis",cut);
...
double FitSpectrum(TTree *&dataraw, double beame, const char* namesfx)
{
 ...
 //RooCBShape cbshape1("cbshape1","crystal ball",x,mean2,sigma2,alpha1,nnn1);
   ...
   RooRealVar meanb("meanb","mean of gaussian",beame,beame-0.005,beame+0.003);
