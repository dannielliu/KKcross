...
int main(int argc, char** argv)
{
  ...
  //else Ana();
  TFile *fileout = new TFile("output/output_bck.root","update");
...
double FitSpectrum3(TTree *&dataraw, double beame, const char* namesfx)
{
   ...
   dataraw->Draw("x1>>hp");
   char name[100];
   sprintf(name,"hp_mcKK2_%.4f",beame*2);
   hp->Write(name);
   delete hp;
   return 0;
...
// RooCBShape cbshape("cbshape1","crystal ball",x,mean,sigma,alpha1,nnn1);
// RooRealVar sigma2("sigma2","width of gaussian",0.01,0.005,0.02);
...
// RooAddPdf sig("sig","signal",RooArgList(cbshape,gaus),RooArgList(frac1));
   ...
   RooRealVar meanb("meanb","mean of gaussian",pmu,pmu-0.002,pmu+0.002);
 ...
 //  RooGaussian gausb("gausb","gauss(x,m,s)",x,meanb,sigmab);
   RooRealVar alphab("alphab","#alpha",1.2,0.1,2);
   RooRealVar nnnb("nnnb","n",1,0.1,2);
   RooCBShape cbshapeb("cbshapeb","crystal ball",x,meanb,sigmab,alphab,nnnb);
   //RooRealVar meane("meane","mean of gaussian",beame,beame-0.005,beame+0.003);
   RooRealVar sigmae("sigmae","width of gaussian",0.01,0.006,0.05);
   RooGaussian gause("gause","gauss(x,m,s)",x,meanb,sigmae);
   RooRealVar frac2("frac2","frac2",0.9,0.8,1.0);
   ...
   pt->SetTextFont(42);
   pt->SetTextSize(0.035);
   sprintf(tmpchr,"#mu = %1.6f #pm %1.6f",meanb.getVal(),meanb.getError());
   pt->AddText(tmpchr);
   sprintf(tmpchr,"#sigma = %1.6f #pm %1.6f",sigmab.getVal(),sigmab.getError());
   pt->AddText(tmpchr);
   sprintf(tmpchr,"#chi^{2}/(%d-%d) = %5.6f",nBins,Npar,xframe->chiSquare(Npar));
   pt->AddText(tmpchr);
   pt->Draw();
 ...
 //outf<<beame<<"\t"<<mean.getVal()<<"\t"<<mean.getError()<<std::endl;
   ofstream fitpar("fitpar",std::ios::app);
   //char line[1000];
   fitpar<<fixed<<setprecision(4);
   fitpar<<" ene = "<< beame*2 ;
   fitpar<<" u " << meanb.getVal()<<" ue= " << meanb.getError();
   fitpar<<" s " << sigmab.getVal()<< " se= " << sigmab.getError();
   fitpar<<" a " << alphab.getVal()<< " ae= " << alphab.getError();
   fitpar<<" n " << nnnb.getVal()<< " ne= " << nnnb.getError();
   fitpar<<" chi2 = " << xframe->chiSquare(Npar);
   fitpar<<std::endl;;
   //fitpar<<"\t e mean = " << meane.getVal() << "\t e sigma = " << sigmae.getVal()<<std::endl;
...
double GetEnergy(int run)
{
  ...
  if (run>=42004 && run<=43253) return 2.125; // separated beam
  return -1;
}
