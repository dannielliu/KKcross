...
int Ana(const char *filename, const char* outdir, TFile *fileout)
{
  ...
  cout<<"run and beam energy "<<run<<" "<< Ebeam<<endl;
...
//cutflow<<"p1 fit signal     :"<<sigp1 <<std::endl;
  //cutflow<<"m1 fit signal     :"<<sigm1 <<std::endl;
   ...
   char name[100];
   sprintf(name,"hp_%s",pureName);
   TH1D* hp = new TH1D(name,"p_{K}",100,beamlow, beamup);
   char opt[100];
   sprintf(opt,"x1>>%s",name);
   datasel1->Draw(opt);
   hp->Write();
   delete hp;
...
double GetEnergy(int run)
{
  ...
  if (run>=27147 && run<=27233) return 3.08; // 2012 jpsi
  if (run>=28241 && run<=28266) return 3.08; // 2012 jpsi
  return -1;
}
