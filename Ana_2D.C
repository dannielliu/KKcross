...
int main(int argc, char** argv)
{
  ...
  //else Ana();
  TFile *fileout = new TFile("output/output_2D.root","update");
...
double FitSpectrum3(TTree *&dataraw, double beame, const char* namesfx)
{
  ...
  //dataraw->Draw("x1>>hp");
  //char name[100];
  //sprintf(name,"hp_mcmumu_%.4f",beame*2);
  //hp->Write(name);
  //delete hp;
  //return 0;
   //
   ...
   // try to use roofit
   //
   RooRealVar x1("x1","p1",peakvalue,beamlow,beamup,"GeV/c");
   RooRealVar x2("x2","p2",peakvalue,beamlow,beamup,"GeV/c");
   // signal
   RooRealVar mean("mean","mean of gaussian",peakvalue,peakvalue-0.003,peakvalue+0.003);
   RooRealVar sigma("sigma","width of gaussian",0.005,0.004,0.013);
   ...
   RooGaussian gaus_p1("gaus_p1","gauss(x,m,s)",x1,mean,sigma2);
   RooCBShape cbshape_p1("cbshape1_p1","crystal ball",x1,mean,sigma,alpha1,nnn1);
   RooRealVar frac1_p1("frac1_p1","frac1",0.8,0.1,1.0);
   RooAddPdf sig_p1("sig_p1","signal",RooArgList(cbshape_p1,gaus_p1),RooArgList(frac1_p1));
   
   RooGaussian gaus_p2("gaus_p2","gauss(x,m,s)",x2,mean,sigma2);
   RooCBShape cbshape_p2("cbshape1_p2","crystal ball",x2,mean,sigma,alpha1,nnn1);
   //RooRealVar frac1_p2("frac1_p2","frac1",0.8,0.1,1.0);
   RooAddPdf sig_p2("sig_p2","signal",RooArgList(cbshape_p2,gaus_p2),RooArgList(frac1_p1));

   RooProdPdf sig("sig","signal",sig_p1, sig_p2);
   ...
   RooRealVar nnnb("nnnb","n",0.6,0.1,4.5);
   RooRealVar sigmae("sigmae","width of gaussian",0.01,0.005,0.08);
   
   RooCBShape cbshapeb_p1("cbshapeb_p1","crystal ball",x1,meanb,sigmab,alphab,nnnb);
   RooGaussian gause_p1("gause_p1","gauss(x,m,s)",x1,meanb,sigmae);
   RooRealVar frac2_p1("frac2_p1","frac2",0.9,0.6,1.0);
   RooAddPdf bck_p1("bck_p1","signal",RooArgList(cbshapeb_p1,gause_p1),RooArgList(frac2_p1));
   
   RooCBShape cbshapeb_p2("cbshapeb_p2","crystal ball",x2,meanb,sigmab,alphab,nnnb);
   RooGaussian gause_p2("gause_p2","gauss(x,m,s)",x2,meanb,sigmae);
   //RooRealVar frac2_p2("frac2_p2","frac2",0.9,0.6,1.0);
   RooAddPdf bck_p2("bck_p2","signal",RooArgList(cbshapeb_p2,gause_p2),RooArgList(frac2_p1));
   
   RooProdPdf bck("bck","background",bck_p1,bck_p2);
   ...
   char tmpchr[100];
   sprintf(tmpchr,"data_2trk_%s",namesfx);
   RooPlot* xframe = x1.frame(Title(""));
   RooPlot* yframe = x2.frame(Title(""));
   ...
   dataset = new RooDataSet(tmpchr,"data",RooArgSet(x1,x2),Import(*dataraw));
   sum = new RooAddPdf("sum","sum",RooArgList(sig,bck),RooArgList(signal,background));
   Npar = 14;
   sum->fitTo(*dataset);
   //x.setRange("sigragi",peakvalue-0.1,beame+0.012);
   //sum->fitTo(*dataset);
   x2.setRange("ysig",0,peakvalue+0.036);
   dataset->plotOn(xframe,CutRange("ysig"));
   sum->plotOn(xframe,Components(sig),ProjectionRange("ysig"),LineStyle(2),LineColor(2));
   sum->plotOn(xframe,Components(bck),ProjectionRange("ysig"),LineStyle(2),LineColor(3));
   //sum->plotOn(xframe,ProjectionRange("ysig"));
   //sum->plotOn(xframe);
   xframe->Draw();
   ...
   c1->SetName(tmpchr);
   c1->Write();
   sprintf(tmpchr,"p_spectrum_xprojection_%s.pdf",namesfx);
   c1->Print(tmpchr);
   
   
   yframe->SetTitle("");
   yframe->GetXaxis()->SetTitle("p (GeV/c)");
   yframe->GetXaxis()->SetNdivisions(505);
   yframe->GetXaxis()->SetTitleSize(0.05);
   yframe->GetXaxis()->SetLabelSize(0.05);
   yframe->GetXaxis()->SetTitleOffset(1.1);
   yframe->GetYaxis()->SetNdivisions(505);
   yframe->GetYaxis()->SetTitleSize(0.05);
   yframe->GetYaxis()->SetTitleOffset(1.25);
   yframe->GetYaxis()->SetLabelSize(0.05);
   yframe->GetYaxis()->SetNoExponent(0);
   x1.setRange("xsig",0,peakvalue+0.036);
   dataset->plotOn(yframe,CutRange("xsig"));
   sum->plotOn(yframe,Components(sig),ProjectionRange("xsig"),LineStyle(2),LineColor(2));
   sum->plotOn(yframe,Components(bck),ProjectionRange("xsig"),LineStyle(2),LineColor(3));
   yframe->Draw();
   pt->Draw();
   sprintf(tmpchr,"p_spectrum_yprojection_%s.pdf",namesfx);
   c1->Print(tmpchr);
   ...
   gStyle->SetOptStat(0);
   //c1->SetLogz();
   TH2D *hp2D = new TH2D("hp2D","2-D p",nBins,beamlow,beamup,nBins,beamlow,beamup);
   TH2D *hp2Derr = new TH2D("hp2Derr","2-D chi",nBins,beamlow,beamup,nBins,beamlow,beamup);
   dataraw->Draw("x1:x2>>hp2D");
   hp2Derr->GetZaxis()->SetRangeUser(-5,5);
   double bingap = (beamup-beamlow)/nBins;
   double sumint = sum->createIntegral(RooArgSet(x1,x2))->getVal();
   double Ntot = signal.getVal()+background.getVal();
   for (int i=1; i<nBins;i++){
     for (int j=1; j<nBins;j++){
       //int gbin = hp2Derr->GetBin(i,j);
       //double dataxy = hp2D->GetBinContent(gbin);
       double dataxy = hp2D->GetBinContent(i,j);
       x1.setVal(beamlow+(i-0.5)*bingap);
       x2.setVal(beamlow+(i-0.5)*bingap);
       double pdfxy = sum->getVal()/sumint;
       double fitxy = pdfxy*Ntot*(bingap*bingap)/(nBins*nBins);
       double chi;
       if (fabs(dataxy)>1) chi = (dataxy-fitxy)/sqrt(dataxy);
       else chi=(dataxy-fitxy);
       hp2Derr->SetBinContent(i,j,chi);
     }
   }
   hp2Derr->Draw("colz");
   sprintf(tmpchr,"chi_%s.pdf",namesfx);
   c1->Print(tmpchr);
 ...
 //fitpar<< std::endl;
   ...
   //x.setRange("sigragi",peakvalue-0.1,beame);
   //x.setRange("sigragi",peakvalue-0.1,beame+0.01);
   x1.setRange("sigrag3",intel3, inteu3);
   x1.setRange("sigrag5",intel5, inteu5);
   RooAbsReal* intsigi = sig.createIntegral( x1,  NormSet(x1),  Range("sigragi"));
   RooAbsReal* intbcki = bck.createIntegral( x1,  NormSet(x1),  Range("sigragi"));
   RooAbsReal* intsig3 = sig.createIntegral( x1,  NormSet(x1),  Range("sigrag3"));
   RooAbsReal* intbck3 = bck.createIntegral( x1,  NormSet(x1),  Range("sigrag3"));
   RooAbsReal* intsig5 = sig.createIntegral( x1,  NormSet(x1),  Range("sigrag5"));
   RooAbsReal* intbck5 = bck.createIntegral( x1,  NormSet(x1),  Range("sigrag5"));
   double signalN3 =   signal.getVal()*intsig3->getVal();
   double backN3 = background.getVal()*intbck3->getVal();
   double signalN5 =   signal.getVal()*intsig5->getVal();
   double backN5 = background.getVal()*intbck5->getVal();
