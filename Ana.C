...
using namespace std;
int Ana(const char *filename=0, const char* outdir=0);
double GetEnergy(int run);
double FitSpectrum(TTree *&dataraw, double beame, const char* namesfx=0);
...
int Ana(const char *filename, const char* outdir)
{
  ...
  for (int ien=0;ien<nentries;ien++){
    ...
    if (fabs(tof11-tof21)<3){
       ...
       if (fabs(p2-pexp)<0.08) {/*datasel2->Fill();*/ count2++; // p2 dis
       datasel1->Fill();
       }
       }
    }
  ...
  std::cout<<"data selction 1 size is "<<datasel1->GetEntries()<<" "<<count1<<std::endl;
  ...
  cutflow<<"After cos<-0.999  :"<<count[2]<<std::endl;
  //cutflow<<"After totp<0.05   :"<<count[3]<<std::endl;
  cutflow<<"After dtof<3      :"<<count3<<std::endl;
  cutflow<<"p1-exp<0.08 for p1:"<<count1<<std::endl;
  cutflow<<"p2-exp<0.08 for p2:"<<count2<<std::endl;
  if (count1==0||count2==0) return -3;
  
  double sigp1=FitSpectrum(datasel1, Ebeam,"p1");
  double sigm1=FitSpectrum2(datasel1, Ebeam,"p1");
...
double getEne(const char* name)
{
  ...
  if (atof(vstr.c_str())>0) return atof(vstr.c_str())/10000;
