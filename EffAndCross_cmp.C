...
int EffAndCross_cmp(char* name=0)
{
  ifstream infile("inicut_new.txt");
  ifstream infile2("inicut_3rd.txt");
  ...
  //ofbes<<defaultfloat;
    ...
    yeff2[poNo] = eff;
    ycor2[poNo] = isrcor;
    yec2[poNo]  = isrcor*eff;
    poNo ++;
    if (poNo>=21) break;
  }
 
  cout<<"ene1 \t cross1 \t cross2 \t diff"<<endl;
  for (int i=0; i<21; i++){
    cout<<x[i]<<" \t "<<y[i]<<" \t "<< y2[i] << " \t "<<(y[i]-y2[i])/y[i]<<endl;
  }
